...
"""
Track 1: x4 downscale and gauss noise std.dev=10 as lq, HR as gt
        [120 x 160] -> [480 x 640]   x4
"""

@DATASET_REGISTRY.register()
class ThermalX4Dataset(data.Dataset):
    """Example dataset.
    ...
    def __init__(self, opt):
        super(ThermalX4Dataset, self).__init__()
        self.opt = opt
    ...
    def __getitem__(self, index):
        ...
        assert img_gt.shape[0] % scale == 0 and img_gt.shape[1] % scale == 0

        # generate lq image
        # downsample
        h, w = img_gt.shape[0:2]

        # ================= add noise according to forum post =========================== #
        noisy_image = img_gt * 255.0 + np.random.normal(0, 10**0.5, img_gt.shape)
        cv2.normalize(noisy_image, noisy_image, 0, 255, cv2.NORM_MINMAX, dtype=-1)
        noisy_image = noisy_image.astype(np.uint8)
        img_lq = cv2.resize(noisy_image, (w // scale, h // scale), interpolation=cv2.INTER_CUBIC)
        img_lq = img_lq / 255.0
        # ================= add noise according to forum post =========================== #


        # ================= add noise according to dataset readme =========================== #
        # img_lq = cv2.resize(img_gt, (w // scale, h // scale), interpolation=cv2.INTER_CUBIC)
        # img_lq = add_gaussian_noise(img_lq, sigma=10, clip=True, rounds=False, gray_noise=False)
        # img_gt_n = add_gaussian_noise(img_gt, sigma=10, clip=True, rounds=True, gray_noise=True)
        # img_lq = cv2.resize(img_gt_n, (w // scale, h // scale), interpolation=cv2.INTER_CUBIC)
        # ================= add noise according to dataset readme =========================== #
